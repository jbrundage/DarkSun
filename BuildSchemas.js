...
const ret = {
    json: {
        registeredUserModel: {
            ...
            logincode:  {type: Number},    /* code to verify the listed user is who they say they are. This gets changed for every new page load */
...
ret.registeredUserModel = mongo.model('registeredUserModel', new mySchema(ret.json.registeredUserModel));
ret.ownedship = mongo.model('ownedship', new mySchema(ret.json.ownedship));
ret.shipkind = mongo.model('shipKind', new mySchema(ret.json.shipkind));
ret.portLocation = mongo.model('portLocation', new mySchema(ret.json.portLocation));
ret.ownedPart = mongo.model('ownedPart', new mySchema(ret.json.ownedPart));
ret.partKind = mongo.model('partKind', new mySchema(ret.json.partKind));
ret.componentLocation = mongo.model('componentLocation', new mySchema(ret.json.componentLocation));
ret.ownedComponent = mongo.model('ownedComponent', new mySchema(ret.json.ownedComponent));
ret.componentKind = mongo.model('componentKind', new mySchema(ret.json.componentKind));
